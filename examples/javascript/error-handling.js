/**
 * Company Name Generator API - Error Handling Example
 *
 * This example demonstrates proper error handling when using the Company Name Generator API.
 * API Documentation: https://docs.apiverve.com/ref/companynamegenerator
 */

const API_KEY = process.env.APIVERVE_API_KEY || 'YOUR_API_KEY_HERE';
const API_URL = 'https://api.apiverve.com/v1/companynamegenerator';
 ...
 */
async function callCompanyNameGeneratorAPIWithErrorHandling(queryParams = {}) {
  try {
...
async function callWithRetry(maxRetries = 3, initialDelay = 1000) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const result = await callCompanyNameGeneratorAPIWithErrorHandling({
